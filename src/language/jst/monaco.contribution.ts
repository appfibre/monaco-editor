...
export interface DiagnosticsOptions {
	...
	readonly schemas?: {
		...
		readonly uri: string;
		/**
		 * A list of glob patterns that describe for which file URIs the JST schema will be used.
...
export const jstDefaults: LanguageServiceDefaults = new LanguageServiceDefaultsImpl(
	'jst',
	diagnosticDefault,
...
function getMode(): Promise<typeof mode> {
	if (AMD) {
		return new Promise((resolve, reject) => {
			require(['vs/language/jst/jstMode'], resolve, reject);
		});
	} else {
		return import('./jstMode');
	}
}
...
languages.register({
	...
	extensions: ['.jst', '.'],
	aliases: ['JST', 'jst'],
	mimetypes: ['application/jst']
});

languages.onLanguage('jst', () => {
	getMode().then((mode) => mode.setupMode(jstDefaults));
});
